...
function Index() {
    ...
    const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        setInput('');
        await sanityClient.patch(handlelisteDocId).set({items: [...items, input]}).commit()
        mutate(query);
    }
        ...
        <div>
            ...
            <ul>
                {items.map(ting => {
                    ...
                    return (<li key={ting}><label><input onClick={() => onCheck(ting, !isChecked)} checked={isChecked} type="checkbox"/>{ting}</label></li>);
                })
                }
            </ul>
        </div>
